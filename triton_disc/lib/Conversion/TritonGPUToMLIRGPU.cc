...
void emitInitLoops(OpBuilder& b, func::FuncOp root,
  ...
  auto forOp =
      createParallelAndSetInsPt(b, loc, /*vars=*/vars1, /*lbs=*/{zero},
                                /*ubs=*/{num_elements}, /*steps=*/{one},
                                /*initValues=*/{});

  // SmallVector<Value, 2> vars2;
  // auto for2 = createParallelAndSetInsPt(b, loc, /*vars=*/vars2,
  // /*lbs=*/{zero},
  /*ubs=*/         //{block_size}, /*steps=*/{one},
  /*initValues=*/  //{});

  Value src1 = b.create<memref::LoadOp>(loc, arg0, forOp.getInductionVars()[0]);
  Value src2 = b.create<memref::LoadOp>(loc, arg1, forOp.getInductionVars()[0]);
  Value add = b.create<arith::AddFOp>(loc, src1, src2);
  b.create<memref::StoreOp>(loc, add, result, forOp.getInductionVars()[0]);
}
