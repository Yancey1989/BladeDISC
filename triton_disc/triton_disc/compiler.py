...
import triton_disc as tt_disc


def ttgir_to_cubin(ttgir, capability):
...
def compile(fn, **kwargs):
    print("begin DISC triton.compile...", flush=True)
    ...
    stages = {
        ...
        "cubin": (lambda path: Path(path).read_bytes(),
                  lambda src: ttgir_to_cubin(src, capability))
    ...
    # cache manager
    print(f"signature: {signature}, constants: {constants}", flush=True)
    ...
    for ir, (parse, compile) in list(stages.items())[first_stage:]:
        ...
        module = next_module
    # write-back metadata
    metadata["name"] = "disc_func_kernel"
    print(asm["cubin"], flush=True)
